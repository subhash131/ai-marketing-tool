...
async def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    ...
    try:
        ...
        print("length of interaction history before:", len(interaction_history))
        
        if(len(interaction_history) > 20):
            # Keep only the last 20 interactions
            interaction_history = interaction_history[-20:]
        
        print("length of interaction history after:", len(interaction_history))
...
async def display_state(
    ...
    try:
        ...
        other_keys = [
            ...
            if k not in ["user_name", "Workflow_definition", "interaction_history"]
        ]
        if other_keys:
